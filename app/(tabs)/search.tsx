...
export default function Search() {
  ...
  const handleSearch = useCallback(async () => {
    if (isSearchable) {
      ...
      const verifiedTracks: Track[] = await Promise.all(
        tracks.map(async (track) => {
          track.added = await trackExists(session.userProfile, track.id);
          return track;
        })
      );
      ...
      <View style={styles.criteriaContainer}>
        <View style={styles.searchByContainer}>
          ...
          <View style={styles.pickerContainer}>
            <TouchableOpacity
              onPress={() => handleAddCriteria("artist")}
            ...
            <TouchableOpacity
              onPress={() => handleAddCriteria("genre")}
              style={[styles.criteriaItem]}
            >
              <View style={styles.checkbox}>
            ...
            <View style={styles.separator} />
            <TouchableOpacity
              onPress={() => handleAddCriteria("year")}
              style={styles.criteriaItem}
            >
              <View style={styles.checkbox}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
          {renderHeader()}
          <ScrollablePinCollection itemType="track" items={data} />
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
