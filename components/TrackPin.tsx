...
function getDurationString(duration_ms?: number): string {
  const duration_mins =
    duration_ms === undefined ? 3 : Math.floor(duration_ms / 60000);
  const duration_secs =
    duration_ms === undefined
      ? 33
      : Math.round((duration_ms % (duration_mins * 60000)) / 1000);
  const duration_secs_str =
    duration_secs < 10 ? `0${duration_secs}` : `${duration_secs}`;
...
export default function TrackPin(props: Props) {
  ...
  const addButtonHandler = async () => {
    ...
    try {
      const res = await extractGenresFromTracks(session.accessToken, [
        props.data,
      ]);
      ...
      if (add) {
        await Promise.all(
          genres.map(async (genre) => {
            const playlistID = await removeTracks(session.userProfile, genre, [
              { id: trackID },
            ]);
            await removeSongsFromPlaylist(session.accessToken, playlistID, [
              trackID,
            ]);
          })
        );
        ...
        await Promise.all(
          genres.map(async (genre) => {
            const playlistID = await addTracks(
              session.userProfile,
              genre,
              async () => {
            ...
            await addSongsToPlaylist(session.accessToken, playlistID, [
              trackID,
            ]);
          })
        );
